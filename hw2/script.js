...
let userInput = +prompt("Enter any positive integer number");

if (userInput >= 1){
    let ones = userInput % 10;
    let tens = +Math.floor((((userInput % 100) / 10)));
    let hundreds = (+Math.floor((userInput / 100)));

    alert(`In number ${userInput} there are: ${hundreds} hundreds, ${tens} tens and ${ones} ones`);
}
else{
    alert("Invalid value");
}